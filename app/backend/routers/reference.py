...
_MOCK_DIMENSIONS = [
    ...
    DimensionDefinition(id=2, code="acuracia", name="Acuracia", article="Art. 2, par.2, II", definition="Medida em que a informacao reflete a realidade de forma precisa", implementation="Validacao fonte primaria e monitoramento de rejeicoes BCB via DLT Expectations", databricks_capability="DLT Expectations + IPOC consistency checks", metrics=[DimensionMetricDef(code="taxa_rejeicao_bcb_pct", name="Taxa de Rejeicao BCB", target=0.0, unit="%")]),
    ...
    DimensionDefinition(id=8, code="consistencia", name="Consistencia", article="Art. 2, par.2, VIII", definition="Coerencia entre dados de diferentes fontes e documentos", implementation="DLT expectations cross-field (date_vencimento_after_contrato, saldo_faixas_consistente_3018, gar_categoria_consistent)", databricks_capability="Silver DLT Expectations + Auto Loader validation", metrics=[DimensionMetricDef(code="taxa_consistencia_pct", name="Taxa de Consistencia", target=99.0, unit="%")]),
