<script>
  ...
  const PRESETS = [
    { id: 'default', primary: '#1E3A5F', accent: '#E84C3D' },
    { id: 'slate',   primary: '#1E293B', accent: '#0EA5E9' },
    { id: 'verde',   primary: '#065F46', accent: '#D97706' },
    { id: 'roxo',    primary: '#4C1D95', accent: '#F59E0B' },
    { id: 'amber',   primary: '#78350F', accent: '#F59E0B' },
    { id: 'custom',  primary: '',        accent: ''        },
  ];
  ...
  async function save() {
    ...
    try {
      ...
      if (!res.ok) throw new Error($_('brand.saveError'));
...
<div class="backdrop" onclick={handleBackdropClick} role="dialog" aria-modal="true" aria-label={$_('brand.title')}>
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <section class="section">
        <label class="section-label">{$_('brand.logo')}</label>
        <div class="logo-area">
          <div class="logo-preview-wrap">
            {#if logoPreview}
              <div class="logo-preview has-logo">
                <img src={logoPreview} alt={$_('brand.logoAlt')} class="logo-img" />
              </div>
              <button class="logo-remove-x" onclick={removeLogo} title={$_('brand.removeLogoTitle')} aria-label={$_('brand.removeLogo')}>
            ...
            {:else}
              <div class="logo-preview is-default">
                ...
                <span class="default-label">{$_('brand.default')}</span>
              </div>
            {/if}
          </div>
          <div class="logo-actions">
            <label class="btn-upload">
              ...
              {logoPreview ? $_('brand.changeLogo') : $_('brand.uploadLogo')}
            ...
            {#if logoPreview}
              <button class="btn-use-default" onclick={removeLogo}>
                ...
                {$_('brand.useDefaultLogo')}
              </button>
            {/if}
            <p class="logo-hint">{$_('brand.logoHint')}</p>
          </div>
        </div>
      </section>
      ...
      <!-- System name -->
      <section class="section">
        <label class="section-label" for="brand-name">{$_('brand.systemName')}</label>
        <input
          id="brand-name"
          type="text"
          class="text-input"
      ...
      <!-- Theme presets -->
      <section class="section">
        <label class="section-label">{$_('brand.colorTheme')}</label>
        <div class="presets-grid">
          {#each PRESETS as preset}
            ...
            >
              ...
              <span class="preset-name">{$_(`brand.preset_${preset.id}`)}</span>
        ...
        {#if selectedPreset === 'custom'}
          <div class="custom-colors">
            <div class="color-field">
              <label class="color-label" for="primary-color">{$_('brand.primaryColor')}</label>
            ...
            <div class="color-field">
              <label class="color-label" for="accent-color">{$_('brand.accentColor')}</label>
              <div class="color-input-row">
