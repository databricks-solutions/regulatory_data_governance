...
| `doc4010_generator` | Job (5 tasks) | Gera saldos COSIF sintéticos a partir do 3040, materializa o **batimento inter-CADOC** (`gold.reconciliacao_cosif`) e exporta os **Doc 4010** (Balancete, mensal) e **Doc 4016** (Balanço, semestral) no leiaute XML oficial. Sem validador BACEN (não há binário oficial do 4010/4016 no repo; o BCB publica o bcValidador + XSD) |
...
| `notebooks/doc4010_generator/` | Notebooks | Saldos COSIF derivados do 3040 (setup/plano de contas, ingestão, motor de saldos, reconciliação, exportação XML 4010+4016) |
...
databricks bundle run doc4010_generator     -t dev-azure --profile <your-databricks-profile>  # requer o 3040 já gerado; emite Doc4010 (mensal) + Doc4016 (semestral)
...
│   ├── doc4010_generator/             # 5 etapas: plano de contas, ingestão, motor de saldos, reconciliação, exportação XML (4010 + 4016)
